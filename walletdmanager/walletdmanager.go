...
var (
	...
	// will be set to a random string when starting weakness-service
	rpcPassword = ""

	cmdWalletd     *exec.Cmd
	cmdWeaknessCoind *exec.Cmd

	// WalletdOpenAndRunning is true when weakness-service is running with a wallet open
...
func RequestBalance() (availableBalance float64, lockedBalance float64, totalBalance float64, err error) {
	...
	availableBalance, lockedBalance, totalBalance, err = weaknesscoinwalletdrpcgo.RequestBalance(rpcPassword)
	if err != nil {
...
func RequestAddress() (address string, err error) {
	...
	address, err = weaknesscoinwalletdrpcgo.RequestAddress(rpcPassword)
	if err != nil {
...
func RequestListTransactions() (transfers []weaknesscoinwalletdrpcgo.Transfer, err error) {

	walletBlockCount, _, _, _, err := weaknesscoinwalletdrpcgo.RequestStatus(rpcPassword)
	if err != nil {
		log.Error("error getting block count: ", err)
		return nil, err
	}

	transfers, err = weaknesscoinwalletdrpcgo.RequestListTransactions(walletBlockCount, 1, []string{WalletAddress}, rpcPassword)
	if err != nil {
...
func SendTransaction(transferAddress string, transferAmountString string, transferPaymentID string, transferFeeString string) (transactionHash string, err error) {
	...
	transactionHash, err = weaknesscoinwalletdrpcgo.SendTransaction(transferAddress, transferAmount, transferPaymentID, transferFee, rpcPassword)
	if err != nil {
...
func OptimizeWalletWithFusion() (transactionHash string, err error) {
	...
	transactionHash, err = weaknesscoinwalletdrpcgo.SendFusionTransaction(smallestOptimizedThreshold, []string{WalletAddress}, WalletAddress, rpcPassword)
	if err != nil {
...
func getOptimisedFusionParameters() (largestFusionReadyCount int, smallestOptimizedThreshold int, err error) {
	...
	for {
		fusionReadyCount, _, err := weaknesscoinwalletdrpcgo.EstimateFusion(threshold, []string{WalletAddress}, rpcPassword)
		if err != nil {
...
func GetPrivateKeys() (isDeterministicWallet bool, mnemonicSeed string, privateViewKey string, privateSpendKey string, err error) {
	...
	isDeterministicWallet, mnemonicSeed, err = weaknesscoinwalletdrpcgo.GetMnemonicSeed(WalletAddress, rpcPassword)
	if err != nil {
		log.Error("error requesting mnemonic seed. err: ", err)
		return false, "", "", "", err
	}

	privateViewKey, err = weaknesscoinwalletdrpcgo.GetViewKey(rpcPassword)
	if err != nil {
		log.Error("error requesting view key. err: ", err)
		return false, "", "", "", err
	}

	privateSpendKey, _, err = weaknesscoinwalletdrpcgo.GetSpendKeys(WalletAddress, rpcPassword)
	if err != nil {
...
// SaveWallet saves the sync status of the wallet. To be done regularly so when weakness-service crashes, sync is not lost
func SaveWallet() (err error) {

	err = weaknesscoinwalletdrpcgo.SaveWallet(rpcPassword)
	if err != nil {
...
// StartWalletd starts the weakness-service daemon with the set wallet info
...
// useCheckpoints is true if WeaknessCoind should be run with "--load-checkpoints"
func StartWalletd(walletPath string, walletPassword string, useRemoteNode bool, useCheckpoints bool, daemonAddress string, daemonPort string) (err error) {
	...
	if isWalletdRunning() {
		errorMessage := "weakness-service is already running in the background.\nPlease close it via "
	...
	pathToLogWeaknessCoindCurrentSession := logsFolder + directorySeparator + logWeaknessCoindCurrentSessionFilename
	pathToLogWeaknessCoindAllSessions := logsFolder + directorySeparator + logWeaknessCoindAllSessionsFilename

	pathToWalletd := "./" + walletdCommandName
	pathToWeaknessCoind := "./" + weaknesscoindCommandName
	...
	if isPlatformDarwin {
		...
		pathToWeaknessCoind = pathToAppContents + "/" + weaknesscoindCommandName
		...
		pathToLogWeaknessCoindCurrentSession = pathToAppLibDir + "/" + pathToLogWeaknessCoindCurrentSession
		pathToLogWeaknessCoindAllSessions = pathToAppLibDir + "/" + pathToLogWeaknessCoindAllSessions
		...
		pathToWeaknessCoind = pathToAppDirectory + "/" + weaknesscoindCommandName
		...
		pathToLogWeaknessCoindCurrentSession = pathToAppDirectory + "/" + pathToLogWeaknessCoindCurrentSession
		pathToLogWeaknessCoindAllSessions = pathToAppDirectory + "/" + pathToLogWeaknessCoindAllSessions
		if pathToWallet == WalletFilename {
			// if comes from createWallet, so it is not a full path, just a filename
	...
	var weaknessCoindCurrentSessionLogFile *os.File
	...
	if !useRemoteNode && !isWeaknessCoindRunning() {

		weaknessCoindCurrentSessionLogFile, err = os.Create(pathToLogWeaknessCoindCurrentSession)
		if err != nil {
			log.Error(err)
		}
		defer weaknessCoindCurrentSessionLogFile.Close()

		if useCheckpoints {
			cmdWeaknessCoind = exec.Command(pathToWeaknessCoind, "--load-checkpoints", pathToCheckpointsCSV, "--log-file", pathToLogWeaknessCoindCurrentSession)
		} else {
			cmdWeaknessCoind = exec.Command(pathToWeaknessCoind, "--log-file", pathToLogWeaknessCoindCurrentSession)
		}
		hideCmdWindowIfNeeded(cmdWeaknessCoind)

		weaknessCoindAllSessionsLogFile, err := os.OpenFile(pathToLogWeaknessCoindAllSessions, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600)
		if err != nil {
			log.Error(err)
		}
		cmdWeaknessCoind.Stdout = weaknessCoindAllSessionsLogFile
		defer weaknessCoindAllSessionsLogFile.Close()

		err = cmdWeaknessCoind.Start()
		if err != nil {
			log.Error(err)
			return err
		}

		log.Info("Opening WeaknessCoind and waiting for it to be ready.")

		readerWeaknessCoindLog := bufio.NewReader(weaknessCoindCurrentSessionLogFile)

		for {
			line, err := readerWeaknessCoindLog.ReadString('\n')
			if err != nil {
				if err != io.EOF {
					log.Error("Failed reading WeaknessCoind log file line by line: ", err)
				}
			}
			if strings.Contains(line, "Imported block with index") {
				log.Info("WeaknessCoind importing blocks: ", line)
			}
			if strings.Contains(line, "Core rpc server started ok") {
				log.Info("WeaknessCoind ready (rpc server started ok).")
				break
			}
			if strings.Contains(line, "Node stopped.") {
				errorMessage := "Error WeaknessCoind: 'Node stopped'"
	...
	log.Info("Opening weakness-service and waiting for it to be ready.")
	...
	for !successLaunchingWalletd && time.Duration(timesCheckLog)*timeBetweenChecks < maxWaitingTime {
		...
		for {
			...
			if strings.Contains(line, "Wallet loading is finished.") {
				...
				log.Info("weakness-service ready ('Wallet loading is finished.').")
				break
			}
			if strings.Contains(line, "Imported block with index") {
				timesCheckLog = 0
			...
			if strings.Contains(line, "INFO    Stopped") {
				...
				if len(listWalletdErrors) > 0 {
					...
					errorMessage = "weakness-service stopped with unknown error"
				}
	...
	_, _, _, _, err = weaknesscoinwalletdrpcgo.RequestStatus(rpcPassword)
	if err != nil {
		killWalletd()
		return errors.New("error communicating with weakness-service via rpc")
	}
...
func GracefullyQuitWalletd() {
	...
	if WalletdOpenAndRunning || cmdWalletd != nil {
		...
		if isPlatformWindows {
			...
			weaknesscoinwalletdrpcgo.SaveWallet(rpcPassword)
			time.Sleep(3 * time.Second)

			err = cmdWalletd.Process.Kill()
			if err != nil {
				log.Error("failed to kill weakness-service: " + err.Error())
			} else {
				log.Info("weakness-service killed without error")
			}
		} else {
			...
			case <-time.After(5 * time.Second):
				if err := cmdWalletd.Process.Kill(); err != nil {
					log.Warning("failed to kill weakness-service: " + err.Error())
				}
				log.Info("weakness-service killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("weakness-service finished with error: " + err.Error())
				}
				log.Info("weakness-service killed without error")
			}
		}
...
// GracefullyQuitWeaknessCoind stops the WeaknessCoind daemon
func GracefullyQuitWeaknessCoind() {

	if cmdWeaknessCoind != nil {
		var err error

		if isPlatformWindows {
			// because syscall.SIGTERM does not work in windows. We have to kill WeaknessCoind.

			err = cmdWeaknessCoind.Process.Kill()
			if err != nil {
				log.Error("failed to kill WeaknessCoind: " + err.Error())
			} else {
				log.Info("WeaknessCoind killed without error")
			}
		} else {
			_ = cmdWeaknessCoind.Process.Signal(syscall.SIGTERM)
			done := make(chan error, 1)
			go func() {
				done <- cmdWeaknessCoind.Wait()
			}()
			select {
			case <-time.After(5 * time.Second):
				if err := cmdWeaknessCoind.Process.Kill(); err != nil {
					log.Warning("failed to kill WeaknessCoind: " + err.Error())
				}
				log.Info("WeaknessCoind killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("WeaknessCoind finished with error: " + err.Error())
				}
				log.Info("WeaknessCoind killed without error")
			}
		}
	...
	cmdWeaknessCoind = nil
}

// CreateWallet calls weakness-service to create a new wallet. If privateViewKey, privateSpendKey and mnemonicSeed are empty strings, a new wallet will be generated. If they are not empty, a wallet will be generated from those keys or from the seed (import)
// walletFilename is the filename chosen by the user. The created wallet file will be located in the same folder as weakness-service.
...
func CreateWallet(walletFilename string, walletPassword string, walletPasswordConfirmation string, privateViewKey string, privateSpendKey string, mnemonicSeed string, scanHeight string) (err error) {
	...
	if WalletdOpenAndRunning {
		return errors.New("weakness-service is already running. It should be stopped before being able to generate a new wallet")
	}
...
func RequestConnectionInfo() (syncing string, walletBlockCount int, knownBlockCount int, localDaemonBlockCount int, peerCount int, err error) {
	...
	walletBlockCount, knownBlockCount, localDaemonBlockCount, peerCount, err = weaknesscoinwalletdrpcgo.RequestStatus(rpcPassword)
	if err != nil {
		return "", 0, 0, 0, 0, err
	}
...
func RequestFeeinfo() (nodeFee float64, err error) {
	...
	_, nodeFee, _, err = weaknesscoinwalletdrpcgo.GetFeeInfo(rpcPassword)
	if err != nil {
		return 0, err
	}
...
func isWeaknessCoindRunning() bool {

	if _, _, err := findProcess(weaknesscoindCommandName); err == nil {
		return true
	}

	if isPlatformWindows {
		if _, _, err := findProcess(weaknesscoindCommandName + ".exe"); err == nil {
			return true
		}
	}
