...
// Package weaknesscoinwalletdrpcgo handles the the rpc connection between your app and weakness-service
package weaknesscoinwalletdrpcgo
...
// returned balances are expressed in WEAK, not in 0.01 WEAK
...
// RequestStatus requests weakness-service connection and sync status
...
// parameters amount and fee are expressed in WEAK, not 0.01 WEAK
func SendTransaction(addressRecipient string, amount float64, paymentID string, fee float64, rpcPassword string) (transactionHash string, err error) {
	...
	amountInt := uint64(amount * 100) // expressed in hundredth of WEAK
	feeInt := uint64(fee * 100)       // expressed in hundredth of WEAK
...
// threshold is the value that determines which outputs will be optimized. Only the outputs, lesser than the threshold value, will be included into a fusion transaction (threshold is expressed in WEAK, not 0.01 WEAK).
...
func EstimateFusion(threshold int, addresses []string, rpcPassword string) (fusionReadyCount int, totalOutputCount int, err error) {
	...
	threshold *= 100 // expressed in hundredth of WEAK
...
// returned fee is expressed in WEAK, not in 0.01 WEAK
