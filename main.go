...
var (
	transfers                   []weaknesscoinwalletdrpcgo.Transfer
	remoteNodes                 []node
	...
	stringBackupKeys            = ""
	rateUSDWEAK                 float64 // USD value for 1 WEAK
...
func main() {
	...
	walletdmanager.GracefullyQuitWeaknessCoind()
}
	...
	if err == nil {
		...
		if needFullUpdate {
			...
			for index, transfer := range transfers {
				...
				amountString += " WEAK (fee: " + strconv.FormatFloat(transfer.Fee, 'f', 2, 64) + ")"
...
func transfer(transferAddress string, transferAmount string, transferPaymentID string, transferFee string) {
	...
	qmlBridge.FinishedSendingTransaction()
	qmlBridge.DisplayPopup("WEAKs sent successfully", 4000)
}
...
func startWalletWithWalletInfo(pathToWallet string, passwordWallet string) bool {
	...
	if err != nil {
		log.Warn("error starting weakness-service with provided wallet info. error: ", err)
		qmlBridge.FinishedLoadingWalletd()
		qmlBridge.DisplayErrorDialog("Error opening wallet.", err.Error())
		return false
	}

	log.Info("success starting weakness-service")
...
func requestRateWEAK() {
	response, err := http.Get(urlCryptoCompareWEAK)
	...
	if err != nil {
		...
		if err != nil {
			...
			if err := json.Unmarshal(b, &resultInterface); err != nil {
				...
				rateUSDWEAK = resultsMap["USD"].(float64)
			}
		}
	}
}
...
func amountStringUSDToWEAK(amountWEAKString string) string {
	amountWEAK, err := strconv.ParseFloat(amountWEAKString, 64)
	if err != nil || amountWEAK <= 0 || rateUSDWEAK == 0 {
		return ""
	}
	amountUSD := amountWEAK * rateUSDWEAK
